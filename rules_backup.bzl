...
def glslang_impl(ctx):
    ...
    for src in ctx.files.srcs:
      ...
      print("C:/VulkanSDK/1.1.92.1/Bin/glslangValidator.exe -V100 %s -o %s" % (src.path, output.path))
      ctx.actions.run(
          inputs = [src],
          outputs = [output],
          executable = "C:/VulkanSDK/1.1.92.1/Bin/glslangValidator.exe",
          arguments = ["-V100 %s -o %s" % (src.path, output.path)],
      )
